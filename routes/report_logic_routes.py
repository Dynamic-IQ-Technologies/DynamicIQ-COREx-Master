...
def generate_pdf_report(report_data, report_info):
    ...
    elements.append(Paragraph(report_info.get('name', 'Report'), title_style))
    elements.append(Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles['Normal']))
    elements.append(Paragraph(f"Report #: {report_info.get('report_number', 'N/A')}", styles['Normal']))
    elements.append(Spacer(1, 20))
    
    if report_data.get('summary'):
        elements.append(Paragraph("Summary", styles['Heading2']))
        summary_text = "<br/>".join([f"<b>{k.replace('_', ' ').title()}:</b> {v:,.2f}" if isinstance(v, float) else f"<b>{k.replace('_', ' ').title()}:</b> {v}" for k, v in report_data['summary'].items()])

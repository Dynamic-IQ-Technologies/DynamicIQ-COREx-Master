...
-   **Tools Management**: Complete tool and equipment tracking system under Procurement. Features tool registration with categories (Hand Tool, Power Tool, Measuring, Calibrated, Safety, Specialty), manufacturer/model/serial tracking, location management, checkout/checkin workflow for employees, calibration scheduling with due date alerts, condition tracking (Good/Fair/Poor), and purchase cost recording. Includes KPI dashboard showing available, in-use, and calibration due counts.
-   **RFQ (Request for Quotation) Module**: Comprehensive RFQ workflow for procurement. Features include multi-line RFQ creation, supplier selection and management, RFQ issuance workflow, quote collection from multiple suppliers per line item, quote comparison and selection, and RFQ closing. Supports full RFQ lifecycle (Draft → Issued → Quotes Received → Closed) with audit logging.

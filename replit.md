...
- **Supply Chain Risk Radar**: Real-time risk intelligence engine scoring every supplier (0-100) and part (0-100) using internal ERP data. Supplier scoring factors: OTIF delivery rate (35pts), lead-time variance (20pts), open quality incidents (15pts), active shortage exposure (20pts), overdue POs (10pts). Part scoring: shortage severity (40pts), primary supplier risk (30pts), urgency vs lead time (20pts), single-source concentration (10pts). AI-generated narrative per supplier (GPT-4o-mini). Async background recalculation with polling. Risk events logged for threshold breaches. Dashboard at `/risk-radar`. Risk badges embedded in Material Requirements Report and Supplier view pages. New DB tables: `supply_risk_profiles`, `risk_signals`, `risk_events`, `risk_score_history`.
...
- **AI Customer Service Agent**: GPT-4o powered customer communication assistant at `/customer-service/ai-agent`. Automatically generates personalized emails based on customer order history, escalations, and recent communications. Supports 8 scenarios (status update, overdue alert, quote follow-up, check-in, shipment notification, order complete, escalation resolution, custom). Drafts are queued for human review with approve/edit/reject workflow. Approved emails are sent via Brevo. Includes bulk auto-scan to generate status updates for all at-risk customers in one click. New columns added to `customer_communications`: `ai_generated`, `ai_status`, `ai_context`, `email_body`, `sent_at`.

...
-   **Sales Module**: Comprehensive sales order management including customer CRUD, various order types (Outright Sales, Exchanges, Managed Repair), advanced line management with workflow, tax calculation, inventory integration (including serialized products), a 5-state order workflow, and robust validation (stock, pricing, discounts, credit limits). Includes Core Due Tracking for exchanges. **Line-Level Inventory Allocation**: Individual line items track allocated inventory with fields for allocated_quantity, allocation_status (Pending/Allocated/Partially Allocated/Backordered), released_to_shipping_at timestamp, and shipped_quantity. Features allocation buttons per line, visual status indicators with color-coded badges, "Allocate All Lines" bulk operation, and validation preventing release of unallocated items. **Line-Level Release to Shipping**: Each line can be individually released to shipping queue after allocation, supporting partial shipments and mixed-status orders where some lines ship while others remain pending. Role-based controls restrict allocation/release to Admin and Planner roles.

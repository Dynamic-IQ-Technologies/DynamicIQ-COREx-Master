...
- **Enterprise Forensic Intelligence (Traceability Engine)**: Cross-module traceability system that reconstructs complete operational history for any Part Number, Serial Number, Lot/Batch Number, Work Order, Purchase Order, or Sales Order. Traces across all modules (Inventory, Manufacturing, Finance, Procurement, Sales, Quality, Shipping, Engineering/BOM, Labor). Features Timeline View, Relationship Graph View, Cost Evolution Chart, Supplier Dependency Snapshot, AI Risk Index scoring, and forensic audit export (JSON/CSV). Routes: `/trace/`, `/trace/search`, `/trace/quick/<type>/<value>`. Services: `services/traceability_engine.py`. Template: `templates/trace/dashboard.html`.
